...
2. В Popular вообше не работают лайки (работают но не обновляются)
3. При получении левела если ошибка, то все ломается. В целом везде провести работу по отлавливанию ошибок.
4. Исправить анимацию в ListQuote чтобы при каждом взаимодействии не дергалас
5. Провести рефакторинг серверной части. Оптимизировать код. Сейчас есть лишние моменты
6. Сделать анимацию для попапов
7. Сделать чтобы не пускало на страницу профиля без авторизации (сейчас можно перейти из строки бразура)
8. Сделать так чтобы логин нельзя был с пробелами использовать
9. Добавить поиск на сам сайт

# Закомитить

1. Доработал маршруты если пользователь не авторизован и отображение попапа авторизации если пользователь пытается зайти на страницу профиля / Improved the routing logic for unauthorized users and added an auth popup when trying to access the profile page.
2. Лайк не обновляется по клику в QuotesCard / Fixed the like button update in QuotesCard without page refresh.
